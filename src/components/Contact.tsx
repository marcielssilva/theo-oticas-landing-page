...
const contacts = [
  {
    ...
    label: "Endereço",
    value: "Rua Coronel Pedro Dias Batista, BOX 04\nMercadão Municipal — Centro\nItapetininga — SP",
    href: "https://maps.google.com/?q=Mercad%C3%A3o+Municipal+Itapetininga",
    cta: "Ver no mapa",
  },
  {
    icon: (
      ...
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {contacts.map((c) => (
            <div
              key={c.label}
              className="group bg-card border border-border rounded-2xl p-8 text-center hover:border-gold/50 hover:shadow-[var(--shadow-gold)] transition-all duration-300"
            >
              <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-gold/10 border border-gold/30 text-gold mb-5 group-hover:bg-gold/20 transition-colors">
                {c.icon}
              </div>
              <p className="text-muted-foreground text-xs tracking-widest uppercase mb-2">
                {c.label}
              </p>
              <p className="text-foreground text-sm font-medium whitespace-pre-line mb-5 leading-relaxed">
                {c.value}
              </p>
              <a
                href={c.href}
                ...
                className="inline-flex items-center gap-1.5 text-gold hover:text-gold-light text-sm font-semibold border-b border-gold/40 hover:border-gold/70 pb-0.5 transition-colors"
              >
                {c.cta} →
              </a>
            </div>
